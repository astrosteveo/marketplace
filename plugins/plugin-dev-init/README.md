...
4. **Plugin Structure** - Plugin organization and manifest configuration
5. **Plugin Settings** - Configuration patterns using .claude/plugin-name.local.md files
6. **Command Development** - Creating slash commands with frontmatter and arguments
7. **Agent Development** - Creating autonomous agents with AI-assisted generation
8. **Skill Development** - Creating skills with progressive disclosure and strong triggers
...
**Trigger phrases:** "add LSP server", "configure language server", "add code intelligence", "set up .lsp.json", "go to definition", "find references"

**What it covers:**
- LSP server configuration (.lsp.json vs plugin.json)
- Required and optional configuration fields
- Common language servers (Go, Python, TypeScript, Rust, etc.)
- Transport options (stdio, socket)
- Debug logging and crash recovery
- Plugin integration with ${CLAUDE_PLUGIN_ROOT}

**Resources:**
- Core SKILL.md (~1,500 words)
- 3 example configurations (Go, Python, multi-language)

**Use when:** Adding code intelligence to your plugin, configuring language servers, or providing real-time diagnostics.
...
- **Core Skills**: 8 SKILL.md files covering all plugin components
- **Reference Docs**: Detailed guides for advanced patterns
- **Examples**: 15+ working examples (hook scripts, MCP configs, LSP configs, plugin layouts, settings files)
- **Utilities**: Production-ready validation/testing/parsing scripts
